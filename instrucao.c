...
void tableswitch() {
    ...
    pcTemp = frameCorrente->pc;

    indice = popOp();

    preenchimento = (4 - ((pcTemp + 1) % 4) ) % 4;
    pcTemp += preenchimento;
    pcTemp++;

    defaultCase = 0;
    for (int l = 0; l < 4; l++) {
        defaultCase = (defaultCase << 8) + frameCorrente->code[pcTemp];
        pcTemp++;
    }

    low = 0;
    for (int l = 0; l < 4; l++) {
        low = (low << 8) + frameCorrente->code[pcTemp];
        pcTemp++;
    }

    if (indice < low && !definido) {
        definido = 1;
        pcNovo = frameCorrente->pc + default_v;
    }

    high = 0;
    for (int l = 0; l < 4; l++) {
        high = (high << 8) + frameCorrente->code[pcNovo];
        pcNovo++;
    }
    ...
    quantOffsets = 1 + high - low;
    posicao = indice - low;
    for (int32_t l = 0; l < quantOffsets; l++) {
        if (l == posicao) {
            offset = 0;
            for (int i = 0; i < 4; i++) {
                offset = (offset << 8) + frameCorrente->code[pcNovo];
                pcNovo++;
            }
